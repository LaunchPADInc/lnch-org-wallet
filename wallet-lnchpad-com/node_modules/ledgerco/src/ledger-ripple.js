...
var LedgerRipple = function(comm) {
	this.comm = comm;
	this.comm.setScrambleKey('r1p');
}

LedgerRipple.prototype.getAddress_async = function(path, boolDisplay, boolChaincode, boolEd25519) {
	var splitPath = utils.splitPath(path);
	var buffer = new Buffer(5 + 1 + splitPath.length * 4);
	var curveMask = (boolEd25519 ? 0x80 : 0x40);
	buffer[0] = 0xe0;
	buffer[1] = 0x02;
	buffer[2] = (boolDisplay ? 0x01 : 0x00);
	buffer[3] = curveMask | (boolChaincode ? 0x01 : 0x00);
...
LedgerRipple.prototype.signTransaction_async = function(path, rawTxHex, boolEd25519) {
	var splitPath = utils.splitPath(path);
	var offset = 0;
...
LedgerRipple.prototype.getAppConfiguration_async = function() {
